...
/datum/extension/network_device/mainframe/proc/delete_file(datum/computer_file/F, list/accesses, mob/user, force = FALSE)
	var/obj/item/stock_parts/computer/hard_drive/HDD = get_storage()
	if(HDD)
		return HDD.remove_file(F, accesses, user, forced = force)
