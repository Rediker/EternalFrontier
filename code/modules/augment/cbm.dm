...
/obj/item/device/compact_bionic_module/attack_self(mob/living/user)
	...
	if (!user.buckled && !user.lying && !cbm_debug)
		...
		if (!affected)
			...
			return
		else if ((loaded_augment.augment_flags & AUGMENTATION_ORGANIC))
			return
			...
			var/obj/item/organ/internal/I = H.internal_organs[loaded_augment.organ_tag]
